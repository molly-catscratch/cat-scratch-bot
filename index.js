  ...
  if (page === 'scheduled') {
    ...
    if (scheduledMessages.length === 0) {
      ...
      scheduledMessages.forEach(msg => {
        const nextRun = msg.repeat === 'none' ? `${msg.date} at ${formatTimeDisplay(msg.time)}` : 
          msg.repeat === 'daily' ? `Daily (weekdays) at ${formatTimeDisplay(msg.time)}` :
          msg.repeat === 'weekly' ? `Weekly (weekdays) at ${formatTimeDisplay(msg.time)}` :
          msg.repeat === 'monthly' ? `Monthly (weekdays) at ${formatTimeDisplay(msg.time)}` :
        ...
        },
        {
          ...
          element: {
            ...
            initial_option: data.repeat === 'daily' ?
              { text: { type: 'plain_text', text: 'Daily (Weekdays)' }, value: 'daily' } :
              data.repeat === 'weekly' ?
              { text: { type: 'plain_text', text: 'Weekly (Weekdays)' }, value: 'weekly' } :
              data.repeat === 'monthly' ?
              { text: { type: 'plain_text', text: 'Monthly (Weekdays)' }, value: 'monthly' } :
          ...
          hint: { type: 'plain_text', text: 'Recurring messages will only run Monday through Friday' }
        }
      );
    }
